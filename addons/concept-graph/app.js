...
function start(check = true)
{
  ...
  links_socket.onmessage = function(event)
  {
    ...
    {
      console.log(msg);
      if( msg.cmd == 'create-concept' )
      {
        if( getNodeById(msg.id) )
        {
          console.log("Duplicate node.");
          return;
        }

        console.log("New concept: " + msg.id, msg);
...
function sendMsgRegister()
{
  ...
  links_socket.send(JSON.stringify(msg));
}
...
{
  return [
    node.icon ? 16 : node.id.length * 4 + 12,
    16
  ];
}
...
var nodes = [
    {id: "Lex Luthor", reflexive: false, x: 200, y: 300},
    {id: "Superman",   reflexive: true,  x: 300, y: 200},
    {id: "Clark Kent", reflexive: false, x: 400, y: 300}
  ],
  lastNodeId = 2,
  links = [
...
function mousedown() {
  ...
  var point = d3.mouse(this),
      node = {id: "Node " + ++lastNodeId, reflexive: false};
  node.x = point[0];
  node.y = point[1];
  nodes.push(node);
